	...
	internal static class TraktTimeZoneHelper {
		...
		private static Dictionary<string, string> OlsonTimeZoneMappings {
			get {
				return _olsonTimeZoneMappings = _olsonTimeZoneMappings ?? new Dictionary<string, string> {
					...
					{"Pacific/Bougainville", "West Pacific Standard Time"},
		...
		internal static TimeZoneInfo FromOlsonTimeZoneId(string olsonTimeZoneId) {
			...
			if (!OlsonTimeZoneMappings.ContainsKey(olsonTimeZoneId)) {
				throw new ArgumentException(string.Format("'{0}' is not a valid Olson time zone ID", olsonTimeZoneId), "olsonTimeZoneId");
			}
