	...
	public static class DateTimeHelper {
		...
		public static DateTime NextOccurrenceOf(string day, string time, TimeZoneInfo timezone = null) {
			if (string.IsNullOrEmpty(day)) {
				throw new ArgumentException("Day not provided", "day");
			}
			if (string.IsNullOrEmpty(day)) {
				throw new ArgumentException("Time not provided", "time");
			}
			if (!Regex.IsMatch(time, @"\d{2}:\d{2}")) {
				throw new ArgumentException("Time must be in the 24 hour format hh:mm", "time");
			}
			...
			while (!ret.ToString("dddd").Equals(day, StringComparison.InvariantCultureIgnoreCase)) {
				ret = ret.AddDays(1);
			}
			if (ret < now) {
				ret = ret.AddDays(7);
