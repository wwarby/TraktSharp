...
namespace TraktSharp.Entities {
	...
	public class TraktShowAirs {
		...
		/// <summary>
		///     The Olson time zone ID for the location in which the show airs (<see cref="Day" /> and <see cref="Time" /> are
		...
		public string LocalDay {
			get {
				if (string.IsNullOrEmpty(Day) || string.IsNullOrEmpty(Time) || !Regex.IsMatch(Time, @"\d{2}:\d{2}") || (TimeZone == null)) {
					return string.Empty;
				}
				...
				if (hours < 0) {
					return dayIndex > 0 ? days[dayIndex - 1] : days.Last();
				}

				if (hours > 24) {
					return dayIndex < 6 ? days[dayIndex + 1] : days.First();
				}
