...
namespace TraktSharp.Modules {
	...
	public class TraktPeopleModule : TraktModuleBase {
		...
		public async Task<TraktPerson> GetPersonAsync(TraktPerson person, TraktExtendedOption extended = TraktExtendedOption.Unspecified) => await GetPersonAsync(person.Ids.GetBestId(), extended);
		...
		public async Task<TraktPerson> GetPersonAsync(string personId, TraktExtendedOption extended = TraktExtendedOption.Unspecified) =>
			await SendAsync(new TraktPeopleSummaryRequest(Client) {
				Id = personId,
				Extended = extended
			});

		/// <summary>
		///     Returns all movies where this person is in the cast or crew. Each cast object will have a character and a
		///     standard movie object.
		/// </summary>
		/// <param name="person">The person</param>
		/// <param name="extended">
		///     Changes which properties are populated for standard media objects. By default, minimal data is
		...
		/// </param>
		/// <returns>See summary</returns>
		public async Task<TraktMovieCredits> GetMoviesForPersonAsync(TraktPerson person, TraktExtendedOption extended = TraktExtendedOption.Unspecified) => await GetMoviesForPersonAsync(person.Ids.GetBestId(), extended);
		...
		public async Task<TraktMovieCredits> GetMoviesForPersonAsync(string personId, TraktExtendedOption extended = TraktExtendedOption.Unspecified) =>
			await SendAsync(new TraktPeopleMoviesRequest(Client) {
				Id = personId,
				Extended = extended
			});
		...
		/// </param>
		/// <returns>See summary</returns>
		public async Task<TraktShowCredits> GetShowsForPersonAsync(TraktPerson person, TraktExtendedOption extended = TraktExtendedOption.Unspecified) => await GetShowsForPersonAsync(person.Ids.GetBestId(), extended);
		...
		public async Task<TraktShowCredits> GetShowsForPersonAsync(string personId, TraktExtendedOption extended = TraktExtendedOption.Unspecified) =>
			await SendAsync(new TraktPeopleShowsRequest(Client) {
				Id = personId,
				Extended = extended
			});
