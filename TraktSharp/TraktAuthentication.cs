...
namespace TraktSharp {
	...
	public class TraktAuthentication {
		...
		public TraktOAuthAccessToken CurrentOAuthAccessToken { get { return _currentOAuthAccessToken = _currentOAuthAccessToken ?? new TraktOAuthAccessToken(); } set => _currentOAuthAccessToken = value; }
		...
		/// <summary>
		///     Parses the uri that Trakt redirects to after successful OAuth authentication to extract the authorization code
		///     and check the anti-forgery token
		/// </summary>
		...
		/// <summary>
		///     Parses the url that Trakt redirects to after successful OAuth authentication to extract the authorization code
		///     and check the anti-forgery token
		/// </summary>
		...
		/// <summary>
		///     Make a call to <see cref="Modules.TraktOAuthModule.GetOAuthTokenAsync()" /> to get an OAuth access token using
		///     <see cref="AuthorizationCode" />, <see cref="ClientId" />,
		///     <see cref="ClientSecret" /> and <see cref="OAuthRedirectUri" />, and parse the result into
		///     <see cref="CurrentOAuthAccessToken" />
		/// </summary>
		/// <returns>
		///     <see cref="CurrentOAuthAccessToken" />
		/// </returns>
		...
		/// <summary>
		///     Make a call to <see cref="Modules.TraktOAuthModule.GetOAuthTokenAsync()" /> to refresh an OAuth access token using
		///     <see cref="AuthorizationCode" />, <see cref="ClientId" />,
		///     <see cref="ClientSecret" /> and <see cref="OAuthRedirectUri" />, and parse the result into
		///     <see cref="CurrentOAuthAccessToken" />
		/// </summary>
		...
		/// <summary>
		///     Log in to the Trakt API using a username/email and password. This is a special case where the application will is
		///     allowed to use a simpler token based authentication
		///     instead of OAuth. In order to fall under this special use case, you will need to contact the trakt staff and get a
		///     special allowance made.
		/// </summary>
		/// <param name="usernameOrEmail">The user's username or email address</param>
		/// <param name="password">The user's password</param>
		/// <returns>
		///     A simple access token which is stored in <see cref="CurrentSimpleAccessToken" /> and used for subsequent Trakt
		///     API requests
		/// </returns>
		...
		/// <summary>
		///     Log out of the Trakt API. This destroys the access token created by <see cref="LoginAsync" /> on the server
		///     and the client
		/// </summary>
