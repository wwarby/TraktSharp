...
namespace TraktSharp {
	...
	public class TraktAuthentication {
		...
		public async Task<TraktAccessToken> GetAccessToken() { 
			var result = await Client.OAuth.GetOAuthTokenAsync(AuthorizationCode, ClientId, ClientSecret, RedirectUri, TraktEnumHelper.GetDescription(TraktOAuthTokenGrantType.AuthorizationCode));
